...
// Matches the page's own revalidate window, so five minutes of traffic costs
// one request rather than one per render. Short enough that a class added on
// Bookwhen shows up while you're still looking for it.
const REVALIDATE_SECONDS = 300;

// A stuck cursor would otherwise loop forever. Twelve pages of twenty is far
// more sessions than this schedule will ever hold — and it has to cover every
// date of every course, not just the ones shown, or a course would be listed
// with fewer sessions than it runs.
const MAX_PAGES = 12;
...
// A location whose whole address just says "online" pins no real place — that
// is how an online session is entered on Bookwhen, which has no online flag.
const ONLINE_ADDRESS = /^(online|virtual|remote|zoom)\b/i;
...
/**
 * A course ticket buys every date in the series rather than one session, so
 * the events that share one are dates of a single course. Sorted and joined so
 * the key is identical for every date, whatever order Bookwhen lists tickets
 * in. Null when nothing here is a course — a one-off workshop is its own
 * thing, and stays that way.
 */
function courseKey(tickets) {
  const ids = tickets
    .filter((ticket) => ticket.attributes?.course_ticket === true)
...
function normalizeEvent(event, index) {
  ...
  const address = oneLineAddress(location);
  ...
  return {
    ...
    location: address,
    // Bookwhen has no explicit online flag; an event with nowhere to show up
    // to — no location at all, or one whose address only says "Online" — is one
    // that happens on a screen.
    isOnline: !address || ONLINE_ADDRESS.test(address),
    price: fromPrice(tickets),
    courseKey: courseKey(tickets),
